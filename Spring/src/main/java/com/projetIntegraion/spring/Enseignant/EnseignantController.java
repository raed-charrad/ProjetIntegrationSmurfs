package com.projetIntegraion.spring.Enseignant;
...
public class EnseignantController {
    ...
    @GetMapping(value = "/listeEnseignant")
    public String showAll(ModelMap modelMap,
            @RequestParam(name = "page", defaultValue = "0") int page,
            @RequestParam(name = "size", defaultValue = "2") int size) {

        Page<User> listEnseignant = userRepository.findByRolesId(5L, PageRequest.of(page, size));
        Set<Integer> ids = listEnseignant.stream().map(u -> u.getId())
                .collect(java.util.stream.Collectors.toSet());
        List<UserRole> listUserRole = userRoleRepository.findByUserIdInAndRoleId(ids, 5L);
        List<Departement> listDepartement = departementRepository.findAll();

        

        modelMap.addAttribute("listEnseignant", listEnseignant);
        ...
        return "ChefDepartment/Enseignant/listEnseignant";
